...
The current repository stores database, email, and payment credentials directly in `config/settings.py`. Before publishing this repository or running it outside your own environment:
...
The repository uses `config/test_settings.py`, which switches tests to SQLite and relaxes staticfiles strictness for repeatable local and CI execution.
...
For Azure App Service, set the startup command to `python -m config.startup`. That keeps database migrations and static collection in the boot path so the app comes up cleanly after each deployment without depending on a shell script path inside the deployed artifact.
...
- most business logic is concentrated in `bookings/views.py`, which would benefit from service-layer extraction
